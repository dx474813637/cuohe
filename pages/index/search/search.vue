<template>
	<view class="w" :style="{
		...
		<view :style="{
			...
			>
				<view class="subsection-w">
					<u-subsection
						:list="subsection_list" 
						mode="subsection" 
						:activeColor="themeConfig.search.activeColor"
						:inactiveColor="themeConfig.search.inactiveColor"
						:current="subsection_current"
						@change="handleSubsectionChange"
					></u-subsection>
				</view>	
					
			</navBar>
			<view class="search-w u-p-l-20 u-p-r-20 u-p-b-10 u-p-t-10">
				<u-search 
					placeholder="搜索商品" 
					v-model="keyword"
					:showAction="false"
	...
	export default {
		data() {
			return {
				...
				list: uni.getStorageSync('historySearch') || [],
				pan: 's',
				title: '搜索'
			};
		},
		...
		onReady() {
			this.setNavColor()
			this.title = `${this.pan == 's'? '卖盘' : '买盘'}商品搜索`
		...
		methods: {
			...
			handleSearch() {
				...
				uni.navigateTo({
					url: `/pages/index/pan/pan?keyword=${this.keyword}&pan=${this.pan}`
				})
			},
			saveSearch() {
